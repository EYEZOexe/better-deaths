...
public sealed class RecapWindow : Window, IDisposable
{
    ...
    private readonly Dictionary<string, ReplayCanvasSlideState> replayCanvasSlideStateByDeathId = new(StringComparer.Ordinal);
    private readonly Dictionary<string, ReplayMitigationExternalPanelTransitionState> replayMitigationExternalPanelTransitionByDeathId = new(StringComparer.Ordinal);
    ...
    private const float ReplayMitigationOverlayWidthMax = 560.0f;
    ...
    private const float ReplayMitigationExternalPanelInitialVisibility = 0.35f;
    private const float ReplayMitigationExternalPanelSlideOffset = 18.0f;
    ...
    private const float ReviewPaneUnframedHorizontalPadding = 6.0f;
    private const float ReviewPaneUnframedVerticalPadding = 4.0f;
    ...
    private readonly record struct ReplayCanvasSlideState(
        float StartX,
        float CurrentX,
        float TargetX,
    ...
    private enum ReplayMitigationOverlayResizeMode
    {
        None,
        Corner,
    }
    ...
    private void DrawModernShell()
    {
        ...
        if (ImGui.BeginChild("##BetterDeathsModernShell", Vector2.Zero, false, OptionalScrollbarFlags))
        {
            ...
            {
                DrawModernCompactShell(navigationItems);
            }
        }
    ...
    private void DrawModernTopNavigation(IReadOnlyList<MainNavigationItem> navigationItems)
    {
        ...
        for (var index = 0; index < navigationItems.Count; index++)
        {
            ...
            var nextWidth = rowWidth <= 0.0f
                ? width
                : rowWidth + ModernTopNavItemGap + width;
            ...
            {
                ImGui.SameLine(0.0f, ModernTopNavItemGap);
            }

            DrawModernTopNavItem(navigationItems[index], width);
    ...
    private static float GetModernTopNavItemWidth(MainNavigationItem item)
    {
        var badgeText = item.BadgeText ?? (item.Highlight ? ThemeNewBadgeText : null);
        var width = ImGui.CalcTextSize(item.Label).X + (ModernTopNavItemPaddingX * 2.0f);
        if (!string.IsNullOrWhiteSpace(badgeText))
        {
            width += GetInlineNavBadgeSize(badgeText).X + 6.0f;
        }

        return MathF.Max(ModernTopNavItemMinWidth, width);
    }

    private void DrawModernTopNavItem(MainNavigationItem item, float width)
    {
        var selected = currentMainPage == item.Page;
        var start = ImGui.GetCursorScreenPos();
        var size = new Vector2(width, ModernTopNavItemHeight);
        var clicked = ImGui.InvisibleButton($"##ModernTopNav{item.Page}", size);
        ...
        var fill = selected
            ...
            : active
                ? ModernNavButtonActiveColor with { W = 0.50f }
                : hovered
                    ? ModernPanelAltColor with { W = 0.54f }
                    : ModernPanelAltColor with { W = ActiveThemeUsesLightPanels() ? 0.28f : 0.18f };
        var border = selected
            ? ModernAccentColor with { W = 0.88f }
            : ModernPanelBorderColor with { W = hovered ? 0.66f : 0.34f };
        drawList.AddRectFilled(start, end, ImGui.GetColorU32(fill), 3.0f);
        drawList.AddRect(start, end, ImGui.GetColorU32(border), 3.0f);
        if (selected)
        {
            drawList.AddRectFilled(
                new Vector2(start.X, end.Y - 3.0f),
        ...
        var labelWidth = MathF.Max(24.0f, width - (ModernTopNavItemPaddingX * 2.0f) - (badgeSize.X > 0.0f ? badgeSize.X + 6.0f : 0.0f));
        var label = ClipTextToWidth(item.Label, labelWidth);
        var labelSize = ImGui.CalcTextSize(label);
        var contentWidth = labelSize.X + (badgeSize.X > 0.0f ? badgeSize.X + 6.0f : 0.0f);
        var contentX = start.X + MathF.Max(0.0f, (width - contentWidth) * 0.5f);
        var labelPos = new Vector2(contentX, start.Y + MathF.Max(0.0f, (size.Y - labelSize.Y) * 0.5f));
        drawList.AddText(
            labelPos,
            ImGui.GetColorU32(selected ? ModernSelectedButtonTextColor : ModernTextColor),
            label);
    ...
    private void DrawModernCompactHeader(IReadOnlyList<MainNavigationItem> navigationItems)
    {
        ...
        var comboWidth = MathF.Min(190.0f, MathF.Max(130.0f, ImGui.GetContentRegionAvail().X * 0.45f));
        var comboX = ImGui.GetCursorPosX() + ImGui.GetContentRegionAvail().X - comboWidth;
        if (comboX > ImGui.GetCursorPosX() + ImGui.GetStyle().ItemSpacing.X)
        {
    ...
    private static Vector2 GetInlineNavBadgeSize(string text)
    {
        return ImGui.CalcTextSize(text) + new Vector2(12.0f, 3.0f);
    }

    private static void DrawInlineNavBadge(ImDrawListPtr drawList, string text, Vector2 position)
    {
        var size = GetInlineNavBadgeSize(text);
        var min = position;
        var max = position + size;
        var fill = LeadUpGoldColor with { W = ActiveThemeUsesLightPanels() ? 0.92f : 0.86f };
        var border = ActiveThemeUsesLightPanels()
            ? ModernPanelBorderColor with { W = 0.88f }
            : ModernTextColor with { W = 0.20f };
        var textColor = ActiveThemeUsesLightPanels()
            ? ModernShellColor
            : new Vector4(0.08f, 0.08f, 0.08f, 1.0f);

        drawList.AddRectFilled(min, max, ImGui.GetColorU32(fill), 3.0f);
        drawList.AddRect(min, max, ImGui.GetColorU32(border), 3.0f);
        drawList.AddText(
            min + new Vector2(6.0f, 1.0f),
    ...
    private static string BuildReplayHeaderMeta(RecordedPullSummary summary)
    {
        return $"{summary.TerritoryName} | {FormatCombatTimer(summary.PullElapsedSeconds)}";
    }

    private static void DrawReplaySurfaceHeader(string title, string meta)
    {
        var startCursor = ImGui.GetCursorPos();
        var availableWidth = ImGui.GetContentRegionAvail().X;
        var style = ImGui.GetStyle();
        var titleSize = ImGui.CalcTextSize(title);
        var metaSize = ImGui.CalcTextSize(meta);
        ...
        var metaFitsInline = metaSize.X <= MathF.Max(0.0f, availableWidth - titleSize.X - style.ItemSpacing.X);
        if (metaFitsInline)
        {
            var metaX = startCursor.X + MathF.Max(0.0f, availableWidth - metaSize.X);
            ImGui.SetCursorPos(new Vector2(metaX, startCursor.Y));
            ImGui.TextColored(ModernMutedTextColor, meta);
            ImGui.SetCursorPos(restoreCursor);
        }
        else
        {
            var clippedMeta = ClipTextToWidth(meta, availableWidth);
            ImGui.TextColored(ModernMutedTextColor, clippedMeta);
        }

        ImGui.Dummy(new Vector2(1.0f, ReplaySurfaceSectionGap * 0.5f));
        DrawSubtleSeparator();
        ImGui.Dummy(new Vector2(1.0f, ReplaySurfaceSectionGap * 0.5f));
    }
    ...
    private void DrawReviewPane(string id, Vector2 size, Action draw, bool framed = true)
    {
        ImGui.PushStyleColor(ImGuiCol.ChildBg, framed ? WithBackgroundOpacity(ModernPanelColor, currentMainWindowBackgroundOpacity) : Vector4.Zero);
        ImGui.PushStyleColor(ImGuiCol.Border, framed ? ModernPanelBorderColor : Vector4.Zero);
        ImGui.PushStyleColor(ImGuiCol.TableHeaderBg, ModernPanelAltColor);
        ...
        ImGui.PushStyleVar(
            ...
            framed
                ? new Vector2(ReviewPaneHorizontalPadding, 6.0f)
                : new Vector2(ReviewPaneUnframedHorizontalPadding, ReviewPaneUnframedVerticalPadding));
        if (ImGui.BeginChild(id, size, framed, OptionalScrollbarFlags))
        {
            draw();
        }
    ...
    private static void DrawSelectedDeathSectionTitle()
    {
        var startCursor = ImGui.GetCursorPos();
        var availableWidth = ImGui.GetContentRegionAvail().X;
    ...
    private void DrawDeathReplayDisplaySettingsContent(string idSuffix)
    {
        ...
        if (CanFitInlineReplayControl(GetReplayCheckboxWidth("Show classes", style), style))
        {
            ImGui.SameLine();
        }
        ...
        if (CanFitInlineReplayControl(GetReplayCheckboxWidth("Show HP", style), style))
        {
            ImGui.SameLine();
        }
    ...
    private static bool CanFitInlineReplayControl(float requiredWidth, ImGuiStylePtr style)
    {
        return ImGui.GetContentRegionAvail().X >= requiredWidth + style.ItemSpacing.X;
    }

    private static float GetReplayCheckboxWidth(string label, ImGuiStylePtr style)
    {
        return ImGui.GetFrameHeight() + style.ItemInnerSpacing.X + ImGui.CalcTextSize(label).X;
    }

    private static float GetReplayWorldMarkerOpacityControlMinWidth(ImGuiStylePtr style)
    {
        return ImGui.CalcTextSize("World marker opacity").X +
            style.ItemInnerSpacing.X +
            ReplayWorldMarkerOpacitySliderMinWidth;
    }
    ...
    private void DrawDeathReplayWorldMarkerOpacitySlider(string idSuffix)
    {
        ...
        var availableWidth = ImGui.GetContentRegionAvail().X;
        var labelWidth = ImGui.CalcTextSize(label).X;
        var canFitInline = availableWidth - labelWidth - style.ItemInnerSpacing.X >= ReplayWorldMarkerOpacitySliderMinWidth;
        ...
        {
            ImGui.SameLine();
            ImGui.SetNextItemWidth(MathF.Min(ReplayWorldMarkerOpacitySliderMaxWidth, availableWidth - labelWidth - style.ItemInnerSpacing.X));
        }
        else
        {
    ...
    private void KeepOnlyActiveReplayDisplayCache(string activeDeathId)
    {
        ...
        KeepOnlyActiveReplayDisplayCacheEntry(replayCanvasSlideStateByDeathId, activeDeathId);
        KeepOnlyActiveReplayDisplayCacheEntry(replayMitigationExternalPanelTransitionByDeathId, activeDeathId);
        ...
        string focusLabel = "Death")
    {
        ...
        var playWidth = GetReplayControlButtonWidth(playLabel, 44.0f);
        if (DrawThemedActionButton(playLabel, $"ReplayPlayPause{idSuffix}", playWidth))
        {
            if (isPlaying)
            {
                replayPlayingByDeathId[idSuffix] = false;
        ...
        DrawReplayControlSameLineIfFits(GetReplayControlButtonWidth("Reset", 52.0f), 4.0f);
        if (DrawThemedActionButton("Reset", $"ReplayReset{idSuffix}", GetReplayControlButtonWidth("Reset", 52.0f)))
        {
            scrubSeconds = minOffset;
            replayScrubSecondsByDeathId[idSuffix] = scrubSeconds;
            replayPlayingByDeathId[idSuffix] = false;
        ...
        DrawReplayControlSameLineIfFits(GetReplayControlButtonWidth(focusLabel, 52.0f), 4.0f);
        if (focusOffset is null)
        {
            ImGui.BeginDisabled();
        }

        if (DrawThemedActionButton(focusLabel, $"ReplayDeath{idSuffix}", GetReplayControlButtonWidth(focusLabel, 52.0f)) && focusOffset is { } targetOffset)
        {
        ...
        DrawReplayControlSameLineIfFits(46.0f, style.ItemSpacing.X * 2.0f);
        DrawReplaySpeedButton(idSuffix, "0.5x", 0.5f);
        DrawReplayControlSameLineIfFits(46.0f, 4.0f);
        DrawReplaySpeedButton(idSuffix, "1x", 1.0f);
        DrawReplayControlSameLineIfFits(46.0f, 4.0f);
        DrawReplaySpeedButton(idSuffix, "2x", 2.0f);

        DrawReplayControlSameLineIfFits(70.0f, style.ItemSpacing.X * 2.0f);
        var showTrails = GetReplayShowTrails();
    ...
    private static float GetReplayControlButtonWidth(string label, float minimumWidth)
    {
        return MathF.Max(minimumWidth, GetThemedActionButtonWidth(label));
    }

    private static void DrawReplayControlSameLineIfFits(float nextWidth, float spacing)
    {
        if (ImGui.GetContentRegionAvail().X >= nextWidth + spacing)
        {
            ImGui.SameLine(0.0f, spacing);
        }
    }
    ...
    private float GetReplayCanvasDisplayX(string idSuffix, float targetX, float minX, float maxX)
    {
        var clampedMinX = MathF.Min(minX, maxX);
        var clampedMaxX = MathF.Max(minX, maxX);
        targetX = Math.Clamp(targetX, clampedMinX, clampedMaxX);

        if (!replayCanvasSlideStateByDeathId.TryGetValue(idSuffix, out var state))
        {
            replayCanvasSlideStateByDeathId[idSuffix] = new ReplayCanvasSlideState(targetX, targetX, targetX, ReplayCanvasSlideDurationSeconds);
            return targetX;
        }

        var currentX = Math.Clamp(state.CurrentX, clampedMinX, clampedMaxX);
        if (MathF.Abs(state.TargetX - targetX) > 0.5f)
        {
            state = new ReplayCanvasSlideState(currentX, currentX, targetX, 0.0f);
        }
        ...
        var deltaSeconds = Math.Clamp(ImGui.GetIO().DeltaTime, 0.0f, 0.25f);
        var elapsedSeconds = MathF.Min(ReplayCanvasSlideDurationSeconds, state.ElapsedSeconds + deltaSeconds);
        var progress = ReplayCanvasSlideDurationSeconds <= 0.0f
            ? 1.0f
            : Math.Clamp(elapsedSeconds / ReplayCanvasSlideDurationSeconds, 0.0f, 1.0f);
        var easedProgress = progress * progress * (3.0f - (2.0f * progress));
        currentX = state.StartX + ((targetX - state.StartX) * easedProgress);

        if (progress >= 1.0f || MathF.Abs(targetX - currentX) <= 0.5f)
        {
            replayCanvasSlideStateByDeathId[idSuffix] = new ReplayCanvasSlideState(targetX, targetX, targetX, ReplayCanvasSlideDurationSeconds);
            return targetX;
        }

        currentX = Math.Clamp(currentX, clampedMinX, clampedMaxX);
        replayCanvasSlideStateByDeathId[idSuffix] = new ReplayCanvasSlideState(state.StartX, currentX, targetX, elapsedSeconds);
        return currentX;
    }

    private ReplayMitigationExternalPanelDisplay UpdateReplayMitigationExternalPanelDisplay(
        string idSuffix,
        bool targetVisible,
        ...
        ReplayExternalStatusPanelLayout targetLayout)
    {
        ...
        var deltaSeconds = Math.Clamp(ImGui.GetIO().DeltaTime, 0.0f, 0.10f);
        var visibilityStep = ReplayMitigationExternalPanelTransitionSeconds <= 0.0f
            ? 1.0f
            : deltaSeconds / ReplayMitigationExternalPanelTransitionSeconds;
        var currentVisibility = hasState
            ? state!.Visibility
            : targetVisible
                ? ReplayMitigationExternalPanelInitialVisibility
                : 0.0f;
        ...
        Vector2 canvasSize)
    {
        ...
        var dockSide = GetReplayMitigationOverlayDockSide();
        SubmitReplayMitigationOverlayCornerResizeHandle(
            idSuffix,
            overlayStart,
            overlaySize,
        ...
        if (string.Equals(replayMitigationOverlayResizeDraggingId, idSuffix, StringComparison.Ordinal))
        {
            var resized = ResizeReplayMitigationOverlay(
                idSuffix,
                canvasStart,
                canvasSize,
                overlaySize,
    ...
    private void SubmitReplayMitigationOverlayCornerResizeHandle(
        string idSuffix,
        Vector2 overlayStart,
        Vector2 overlaySize,
        ReplayOverlayDockSide dockSide,
        ref bool hovered,
        ref bool active)
    {
        var cornerSize = MathF.Min(ReplayMitigationOverlayResizeCornerSize, MathF.Min(overlaySize.X, overlaySize.Y));
        var cornerX = dockSide == ReplayOverlayDockSide.Left
            ? overlayStart.X + overlaySize.X - cornerSize
            : overlayStart.X;
        ImGui.SetCursorScreenPos(new Vector2(cornerX, overlayStart.Y + overlaySize.Y - cornerSize));
        ImGui.InvisibleButton(
            $"##ReplayMitigationOverlayResizeCorner{idSuffix}",
            new Vector2(cornerSize));
        ...
        if (ImGui.IsItemActivated())
        {
            ...
            replayMitigationOverlayResizeMode = ReplayMitigationOverlayResizeMode.Corner;
            replayMitigationOverlayResizeDockSide = dockSide;
        }
    }

    private (Vector2 Start, Vector2 Size) ResizeReplayMitigationOverlay(
        string idSuffix,
        Vector2 canvasStart,
        Vector2 canvasSize,
        Vector2 overlaySize,
        float minHeight)
    {
        ...
        if (replayMitigationOverlayResizeMode == ReplayMitigationOverlayResizeMode.Corner)
        {
            var widthDelta = replayMitigationOverlayResizeDockSide == ReplayOverlayDockSide.Left
                ? mouseDelta.X
                : -mouseDelta.X;
            newWidth = Math.Clamp(
                overlaySize.X + widthDelta,
                GetReplayMitigationOverlayMinWidth(canvasSize),
                GetReplayMitigationOverlayMaxWidth(canvasSize));
            configuration.ReplayMitigationOverlayWidth = newWidth;
        }

        configuration.ReplayMitigationOverlayHeight = newHeight;
        var resizedSize = new Vector2(newWidth, newHeight);
        ...
        var targetX = GetReplayMitigationOverlayTargetX(canvasStart, canvasSize, resizedSize);
        replayMitigationOverlayAnimatedXByDeathId[idSuffix] = targetX;
        return (
            new Vector2(targetX, canvasStart.Y + configuration.ReplayMitigationOverlayOffsetY),
            resizedSize);
    }
        ...
        IReadOnlyList<ReplayMitigationOverlayEntry> mitigationEntries)
    {
        ...
        var defaultWidth = Math.Clamp(
            canvasSize.X * 0.48f,
            minWidth,
            MathF.Min(ReplayMitigationOverlayDefaultWidthMax, maxWidth));
    ...
    {
        return MathF.Min(ReplayMitigationOverlayWidthMin, GetReplayMitigationOverlayMaxWidth(canvasSize));
    }

    private static float GetReplayMitigationOverlayMaxWidth(Vector2 canvasSize)
    {
        return MathF.Max(48.0f, MathF.Min(ReplayMitigationOverlayWidthMax, canvasSize.X - (ReplayMitigationOverlayPadding * 2.0f)));
    }
    ...
    {
        return GetReplayMitigationOverlayDockSide() == ReplayOverlayDockSide.Left
            ? canvasStart.X + ReplayMitigationOverlayPadding
            : canvasStart.X + canvasSize.X - ReplayMitigationOverlayPadding - overlaySize.X;
    }

    private ReplayOverlayDockSide GetReplayMitigationOverlayDockSide()
    {
        return Enum.IsDefined(configuration.ReplayMitigationOverlayDockSide)
            ? configuration.ReplayMitigationOverlayDockSide
            : ReplayOverlayDockSide.Right;
    }
    ...
    private static Vector4 ScaleAlpha(Vector4 color, float alpha)
    {
        return color with { W = color.W * Math.Clamp(alpha, 0.0f, 1.0f) };
    }
        ...
        float alpha)
    {
        ...
        drawList.AddRectFilled(
            ...
            ImGui.GetColorU32(ModernPanelColor with { W = 0.90f * alpha }),
            6.0f);
        drawList.AddRect(
            start,
            end,
            ImGui.GetColorU32(ModernPanelBorderColor with { W = (hovered || active ? 0.92f : 0.72f) * alpha }),
            6.0f);
    }
        ...
        float alpha = 1.0f)
    {
        ...
        for (var index = 0; index < entries.Count; index++)
        {
            ...
            DrawReplayMitigationOverlayRow(drawList, entry, start.X + ReplayMitigationOverlayPadding, rowY, size.X - (ReplayMitigationOverlayPadding * 2.0f), alpha);
    ...
    private static void DrawReplayMitigationOverlayResizeGrip(
        ImDrawListPtr drawList,
        Vector2 start,
        Vector2 size,
        ReplayOverlayDockSide dockSide,
        bool active)
    {
        ...
        var colorU32 = ImGui.GetColorU32(color with { W = active ? 0.95f : 0.70f });
        var cornerSize = ReplayMitigationOverlayResizeCornerSize - 6.0f;
        var cornerX = dockSide == ReplayOverlayDockSide.Left
            ? start.X + size.X - 5.0f
            : start.X + 5.0f;
        var cornerY = start.Y + size.Y - 5.0f;
        for (var i = 0; i < 3; i++)
        {
            var inset = 3.0f + (i * 4.0f);
            if (dockSide == ReplayOverlayDockSide.Left)
            {
                drawList.AddLine(
                    new Vector2(cornerX - inset, cornerY),
                    new Vector2(cornerX, cornerY - inset),
                    colorU32,
                    1.2f);
            }
            else
    ...
    {
        replayArena = null;
        if (replayModule.TryGetReplayArena(positions, actorStates, selectedAtUtc, out var arena) &&
    ...
    private static void DrawReplayGrid(
        ...
        float zoom,
        Vector2 pan,
        ReplayArenaInfo? replayArena = null)
    {
        ...
        var arenaCenterX = replayArena?.CenterX ?? centerX;
        ...
        else if (arenaShape is ReplayArenaShape.Square or ReplayArenaShape.Rectangle)
        {
            ...
            drawList.AddRect(arenaMin, arenaMax, ImGui.GetColorU32(ModernPanelBorderColor with { W = 0.9f }), 0.0f, ImDrawFlags.None, 1.6f);

            var innerMin = ReplayWorldPointToScreen(arenaCenterX - (arenaHalfWidth * 0.5f), arenaCenterZ - (arenaHalfHeight * 0.5f), canvasStart, canvasSize, minX, maxX, minZ, maxZ, zoom, pan);
            var innerMax = ReplayWorldPointToScreen(arenaCenterX + (arenaHalfWidth * 0.5f), arenaCenterZ + (arenaHalfHeight * 0.5f), canvasStart, canvasSize, minX, maxX, minZ, maxZ, zoom, pan);
            drawList.AddRect(innerMin, innerMax, ImGui.GetColorU32(ModernDividerColor with { W = 0.22f }), 0.0f, ImDrawFlags.None, 1.0f);
        }

        drawList.AddLine(
            ReplayWorldPointToScreen(arenaCenterX - arenaRadius, arenaCenterZ, canvasStart, canvasSize, minX, maxX, minZ, maxZ, zoom, pan),
            ReplayWorldPointToScreen(arenaCenterX + arenaRadius, arenaCenterZ, canvasStart, canvasSize, minX, maxX, minZ, maxZ, zoom, pan),
            ImGui.GetColorU32(ModernDividerColor with { W = 0.26f }),
            1.0f);
        drawList.AddLine(
            ReplayWorldPointToScreen(arenaCenterX, arenaCenterZ - arenaRadius, canvasStart, canvasSize, minX, maxX, minZ, maxZ, zoom, pan),
            ReplayWorldPointToScreen(arenaCenterX, arenaCenterZ + arenaRadius, canvasStart, canvasSize, minX, maxX, minZ, maxZ, zoom, pan),
            ImGui.GetColorU32(ModernDividerColor with { W = 0.26f }),
            1.0f);
